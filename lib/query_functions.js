...
const getMenuForRestaurant1 = function (db) {
  return db.query(`SELECT id, name, description, price FROM menu_items WHERE restaurant_id = 1 ORDER BY name;`)
    .then(res => {
...
const getMenuForRestaurant2 = function (db) {
  return db.query(`SELECT id, name, description, price FROM menu_items WHERE restaurant_id = 2 ORDER BY name;`)
    .then(res => {
      console.log(res.rows[0]);
      return res.rows;
    })
...
const getMenuForRestaurant3 = function (db) {
  return db.query(`SELECT id, name, description, price FROM menu_items WHERE restaurant_id = 3 ORDER BY name;`)
    .then(res => {
      return res.rows;
    })
    .catch(err => {
...
const addOrderForRestaurant = async function (db, obj) {
  try {
    ...
    const idOrd = await db.query(`INSERT INTO orders (date_time_ordered_created, total_amount, restaurant_id, customer_id)
    VALUES (CURRENT_TIMESTAMP, ${obj.totalAmount}, '${restaurantIdForOrder}', '${userIdForOrder}');`);
    // obj.array.forEach( res => {
    //   INSERT ...obj..
    // })
    // const array = await
...
// Adds an order - TESTING PHASE
const addOrderForAny = function (db, totalAmount, restaurantIdForOrder, userId) {
  return db.query(`INSERT INTO orders (date_time_ordered_created, total_amount, restaurant_id, customer_id)
  VALUES (CURRENT_TIMESTAMP, ${totalAmount}, ${restaurantIdForOrder}, ${userId});`)
    .then(res => {
      console.log("TESTING THE ADDITION");
      return res.rows;
    })
    .catch(err => {
      throw err;
