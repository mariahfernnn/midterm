...
// Gets all orders for restaurants
const getOrderInfo = function (db, restaurantNameOfOrder) {
  return db.query(`SELECT * FROM orders
  JOIN restaurants ON orders.restaurant_id = restaurants.id
  JOIN order_items ON orders.id = order_items.order_id
  JOIN menu_items ON menu_items.id = order_items.menu_items_id WHERE restaurants.name = 'Baro' LIMIT 5;`)
    .then(res => {
      return res.rows;
    })
    .catch(err => {
...
// This is used for the order submit button
