...
const getMenuForRestaurant1 = function (db) {
  return db.query(`SELECT * FROM menu_items WHERE restaurant_id = 1 ORDER BY name;`)
    .then(res => {
...
const getMenuForRestaurant2 = function (db) {
  return db.query(`SELECT * FROM menu_items WHERE restaurant_id = 2 ORDER BY name;`)
    .then(res => {
      console.log(res.rows[0]);
      return res.rows;
    })
...
const getMenuForRestaurant3 = function (db) {
  return db.query(`SELECT * FROM menu_items WHERE restaurant_id = 3 ORDER BY name;`)
    .then(res => {
      return res.rows;
    })
    .catch(err => {
...
const addOrderForRestaurant = async function (db, obj) {
  try {
    ...
    VALUES (CURRENT_TIMESTAMP, ${obj.totalAmount}, '${restaurantIdForOrder}', '${userIdForOrder}');`);
    // obj.menuItemsArray.forEach( res => {
    //   const menuItemId = parseInt(Object.keys(res))
    // })
    // const array = await
